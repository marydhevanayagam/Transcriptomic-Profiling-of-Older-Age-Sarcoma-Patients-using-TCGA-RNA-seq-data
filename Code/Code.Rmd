...
#-------------------------------------------------------------------------------------------------------------------------------------------------
#1.5. Obtaining the RNA-seq data analysis
...
write.csv(selectResults.levels, file ="../Documents/DGEA_and_FEA/All_DGEA_results.csv", row.names = F)
...
write.csv(DGEA_sig, file ="../Documents/DGEA_and_FEA/DGEA_sig_results.csv", row.names = F)
...
highlight_genes <- c(DGEA_sig$gene[order(-DGEA_sig$logFC)][1:10], DGEA_sig$gene[order(DGEA_sig$logFC)][1:10])
...
#Plotting Heat map
pdf("../Images/DGEA and FEA/Heatmap.pdf", width = 10, height = 10)
...
heatmap.2(as.matrix((heat.data)), #can use log values for better visualization
          ...
          margins = c(9,10),                # Adjust margins to fit labels
          #main = "Heatmap", #Title
          xlab = "",                 #X axis label
...
mtext("Samples(n=262)", side = 1, line = 1)  # Add custom xlab at line 4
...
#write.csv(Upregulated_genes_DGEA, "../Documents/DGEA_and_FEA/Upregulated_DGEA.csv", row.names = F)
#write.csv(Downregulated_genes_DGEA, "../Documents/DGEA_and_FEA/Downregulated_DGEA.csv", row.names = F)
...
EAbarplot_upreg_genes <- (TCGAvisualize_EAbarplot(tf = rownames(up.EA$ResBP), #Row names
                                                  ...
                                                  filename = "../Images/DGEA and FEA/Up_regulated_FEA.pdf",
                                                  text.size = 2, # 2 
...
EAbarplot_downreg_genes <- (TCGAvisualize_EAbarplot(tf = rownames(dn.EA$ResBP),
                                                    ...
                                                    nBar = 5, 
                                                    filename = "../Images/DGEA and FEA/Down_regulated_FEA.pdf",
                                                    text.size = 2, 
                                                    fig.width = 30,
                                                    fig.height = 15))
#-------------------------------------------------------------------------------------------------------------------------------------------------
...
write.csv(selected_DR_TF, file ="../Documents/Transcription_Factor_Analysis/sig-TF_DoRothEA_analysis.csv",row.names = F)
...
write.csv(merged_data_sig_TF_TR, file ="../Documents/Transcription_factor_TRRUST.csv",row.names = F)
...
write.csv(selected_TF_TR,file="../Documents/Transcription_Factor_Analysis/sig-TF_TRRUST_analysis.csv", row.names = F)
...
#3.4. Functional Enrichment analysis of sig-TF's using ShinyGo ()
...
write.csv(sig_survival, file = "../Documents/Gene_Specific_Survival_Analysis/sig-Survival.csv", row.names = F)
...
#Save the file
ggsave("../Images/Gene_Specific_Survival_Analysis/combined_survival_curves_final.pdf", 
       plot = combined_plot, 
       width = 20,  
       height = 20,  
       dpi = 150)  
